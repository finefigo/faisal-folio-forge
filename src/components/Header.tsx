...
const Header = () => {
  ...
  return <motion.header initial={{
    ...
    stiffness: 100,
    damping: 20
  }} className={`fixed w-full top-0 z-50 transition-all duration-500 ${scrolled ? "bg-tech-blue/90 backdrop-blur-sm shadow-lg py-3" : "py-5"}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <nav className="flex justify-between items-center">
          ...
          <div className="md:hidden">
            <Button variant="ghost" size="icon" onClick={() => setIsMenuOpen(!isMenuOpen)} aria-label="Toggle Menu">
              {isMenuOpen ? <X className="h-6 w-6 text-tech-teal" /> : <Menu className="h-6 w-6 text-tech-teal" />}
            </Button>
          </div>
        </nav>

        {/* Mobile Menu */}
        {isMenuOpen && <motion.div className="md:hidden fixed inset-0 top-[72px] bg-tech-blue/95 backdrop-blur-md z-40" initial={{
        opacity: 0,
        x: 100
      }} animate={{
        opacity: 1,
        ...
        damping: 30
            ...
            <div className="flex flex-col items-center justify-center h-full">
              <motion.ul className="flex flex-col space-y-6 text-center font-medium text-lg" initial="hidden" animate="visible" variants={{
            hidden: {},
            visible: {
              transition: {
                staggerChildren: 0.1
                ...
                {navLinks.map((link, index) => {
              const sectionId = link.url.substring(1);
              return <motion.li key={link.name} className="nav-item" variants={{
                hidden: {
                  opacity: 0,
                  y: 20
                },
                visible: {
                  ...
                  y: 0
                      ...
                      <a href={link.url} className="nav-link" onClick={e => handleNavClick(e, sectionId)}>
            ...
            delay: 0.6
                ...
                <Button asChild className="btn-outline">
                  <a href="/Faisal_Imtiaz_Resume.pdf" target="_blank" rel="noopener noreferrer" onClick={() => setIsMenuOpen(false)}>
                    Resume
                  </a>
                </Button>
              </motion.div>
